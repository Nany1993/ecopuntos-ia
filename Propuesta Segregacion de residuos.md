...
- **Capa de orquestación:** n8n (diseño inicial del piloto) para gestión de sesiones, reglas y flujo de integración. *En la implementación simplificada del prototipo, esta capa fue absorbida por Python (`SessionService` + bot de Telegram); ver sección 4.3.*
- **Capa de inteligencia:** Modelo de IA Gemini (principal) y, opcionalmente, GPT como respaldo.
- **Capa de datos:** Google Sheets (diseño inicial del piloto) y Supabase como alternativa de escalamiento. *En la implementación simplificada, la persistencia quedó en **SQLite** local (`data/smartsort.db`); ver sección 4.3.*
- **Capa analítica:** Looker Studio para visualización de KPIs (diseño inicial). *En el prototipo implementado, los KPIs se visualizan con un **dashboard Streamlit + Plotly** (`src/dashboard/`) conectado a SQLite.*
...
### 4.3 Ajuste de arquitectura: simplificación del prototipo implementado

En el diseño inicial de esta propuesta, **n8n** fue planteado como capa de orquestación entre Telegram, el modelo de IA y la persistencia de datos. Dicha decisión respondía a un enfoque académico que permitía visualizar el flujo de extremo a extremo en un diagrama de workflows y separar claramente las responsabilidades por capas.

Durante la **implementación del prototipo funcional**, se adoptó un **ajuste de arquitectura** con el fin de **simplificar la operación del piloto**, sin alterar la lógica de negocio ni los objetivos del proyecto. En concreto, la orquestación que en el diseño original recaía sobre n8n fue **centralizada en Python**, mediante:

- **Bot de Telegram** (`telegram_bot.py`) como interfaz de captura y retroalimentación.
- **SessionService** como motor de sesiones, reglas de decisión, comparación `prediccion_ia` vs `caneca_qr` e integración con EcoPuntos.
- **classifier.py** como conector directo con **Gemini 2.5 Flash** (y GPT opcional como respaldo).
- **SQLite** (`data/smartsort.db`) como única persistencia del piloto: catálogo de canecas, intentos, EcoPuntos e historial.

**Persistencia simplificada (Google Sheets):** el diseño original contemplaba replicar datos en **Google Sheets** para tableros en Looker Studio. En la implementación final del prototipo se optó por **solo SQLite**, porque concentra toda la trazabilidad en un archivo local, elimina cuentas de servicio de Google Cloud, sincronización en tiempo real y dependencias adicionales (`gspread`, credenciales JSON), sin perder registro de sesiones, intentos ni puntos. Looker Studio y Sheets permanecen como evolución futura si el piloto escala a producción.

**Justificación del cambio (orquestación n8n → Python):**

1. **Menor complejidad operativa:** el piloto puede ejecutarse con un solo proceso (`python -m src.bot.telegram_bot`), en lugar de coordinar simultáneamente n8n, una API intermedia, túneles HTTPS (ngrok) y el bot.
2. **Despliegue más ágil para la demo académica:** se eliminan dependencias adicionales (instalación de n8n, configuración de webhooks, credenciales duplicadas) que no aportan funcionalidad nueva al alcance del prototipo.
3. **Estabilidad en pruebas:** el bot en modo *polling* evita conflictos de webhook y el error 409 de Telegram cuando conviven varios consumidores del mismo token.
4. **Mantenimiento y depuración:** la lógica de sesiones, IA y registro queda en un mismo repositorio, lo que facilita trazabilidad del código, pruebas y corrección de errores durante el piloto.
5. **Equivalencia funcional:** se conservan íntegramente el flujo operativo (QR → foto → clasificación IA → veredicto → confirmación → KPIs), los estados de sesión, el registro en `registro_intentos` y la gamificación **EcoPuntos**.

**Alcance de la simplificación:** n8n **no se descarta como evolución futura** del sistema (por ejemplo, para integrar nuevos canales, automatizar alertas o escalar la orquestación en producción), pero **no fue necesario para validar el prototipo** en el contexto del piloto IoT 2.0. La capa de orquestación queda, en la implementación actual, absorbida por el servicio de aplicación en Python, manteniendo el smartphone como dispositivo IoT, Telegram como canal de interacción y Gemini como agente de clasificación.

---

## 5. Flujo operativo de extremo a extremo (inicio a fin)

> **Nota de implementación:** en el prototipo construido, los pasos descritos a continuación que originalmente se asignaban a n8n son ejecutados por el **bot de Telegram** y el módulo **SessionService** en Python. El flujo lógico se mantiene igual; solo cambia el componente que orquesta las etapas.

1. El colaborador escanea el QR de la caneca.
2. Se abre Telegram con el parámetro `id_caneca`.
3. El sistema crea `id_sesion` y solicita foto del residuo.
4. El colaborador envía la imagen.
5. Se consulta el catálogo de canecas por `id_caneca` y se recuperan `caneca_qr`, `area` y `tipos_residuo_permitidos`.
6. Se invoca el modelo de IA (Gemini) para clasificación del residuo.
7. La IA responde con `prediccion_ia`, `nivel_confianza` y `explicacion_breve`.
8. Se compara `prediccion_ia` contra `caneca_qr`.
...
13. Los indicadores de desempeño se calculan sobre `registro_intentos` en SQLite (consultas en `sql/kpi_queries.sql`).
...
[Telegram abre con id_caneca]
          |
          v
[Sistema crea id_sesion (SessionService)]
          |
          v
...
- **Python** como lenguaje principal del prototipo: bot de Telegram, orquestación de sesiones (`SessionService`), clasificación con IA (`classifier.py`).
- **Orquestación y persistencia simplificadas:** Python + SQLite (véase sección 4.3). n8n y Google Sheets permanecen como referencia de diseño y opción de escalamiento futuro.
- Integración API con IA (Gemini como proveedor principal y GPT como respaldo opcional).
- Telegram Bot API como interfaz de interacción con el usuario (modo polling en el piloto).
- **SQLite** como persistencia del piloto (`data/smartsort.db`).
- Consultas KPI en `sql/kpi_queries.sql` y **dashboard interactivo Streamlit** (`scripts/start_dashboard.ps1`).
- Seguridad básica: variables de entorno para credenciales, control de acceso y seudonimización de identificadores.
...
- Configuración inicial de Telegram Bot y servicios Python (SessionService, classifier).

### Semana 2: Implementación del flujo base
- Desarrollo del flujo QR → Telegram → IA → veredicto (orquestado en Python; diseño original con n8n).
- Integración de registro de intentos en SQLite.
...
- Medición de KPIs por caneca, área y colaborador (consultas SQL sobre SQLite).
